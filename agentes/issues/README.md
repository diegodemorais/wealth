...
|----|--------|------|--------|
| DEV-scheduled-status | Dashboard: status das rotinas agendadas — bloco após Changelog + DiagnosticBanner NOW | 2026-05-06 | `scripts/scheduled_reporter.py` + LaunchAgent io.fdte.scheduled-reporter (seg–sex 7h25). `useScheduledStatus` hook + `<ScheduledStatus>` colapsado após Changelog no Tools tab. DiagnosticBanner condicional na NOW tab (error/stale only). 9 vitest ✓, Playwright smoke ✓, Gate: TS ✓ Build ✓ 130/130 ✓ Vitest 839/839 ✓ Sanity 30/30 ✓. |
