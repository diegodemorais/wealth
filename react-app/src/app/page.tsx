...
export default function HomePage() {
  ...
  return (
    <div>
      ...
      <div className="text-xs uppercase font-semibold text-slate-400 mb-1.5 tracking-widest">Indicadores Primários</div>
      <div className="grid grid-cols-3 gap-2.5 mb-2">
        {/* P(Cenário Aspiracional) */}
        <div className="bg-card border-2 border-blue-500/40 rounded-lg p-4 text-center">
          <div className="text-xs uppercase font-semibold text-slate-400 mb-1 tracking-widest">P(Cenário Aspiracional)</div>
          <div className="text-2xl font-black text-blue-400">{derived.pfireAspiracional != null ? `${derived.pfireAspiracional.toFixed(1)}%` : '—'}</div>
          <div className="text-xs text-slate-400 mt-1">cenário base</div>
        </div>
        {/* Drift Máximo */}
        <div className="bg-card border-2 border-blue-500/40 rounded-lg p-4 text-center">
          <div className="text-xs uppercase font-semibold text-slate-400 mb-1 tracking-widest">Drift Máximo</div>
          <div className="text-2xl font-black text-white">{maxDrift.toFixed(2)}pp</div>
          <div className="text-xs text-slate-400 mt-1">vs alvo IPS</div>
        </div>
        {/* Aporte do Mês */}
        <div className="bg-card border border-slate-700/50 rounded-lg p-4 text-center">
          <div className="text-xs uppercase font-semibold text-slate-400 mb-1 tracking-widest">Aporte do Mês</div>
          ...
          <div className="text-xs text-slate-400 mt-1">{derived.ultimoAporteData || '—'}</div>
        </div>
      </div>

      {/* 3. KPI GRID: Contexto de Mercado — Dólar, Bitcoin, IPCA+ 2040, Renda+ 2065 */}
      <div className="text-xs uppercase font-semibold text-slate-400 mb-1.5 tracking-widest">Contexto de Mercado</div>
      <div className="grid grid-cols-4 gap-2.5 mb-3 opacity-85">
        {/* Dólar */}
        <div className="bg-card border border-slate-700/50 rounded-lg p-3 text-center">
          <div className="text-xs uppercase font-semibold text-slate-400 mb-1 tracking-widest">Dólar</div>
          <div className="text-xl font-black text-white">{derived.CAMBIO ? `R$ ${derived.CAMBIO.toFixed(2)}` : '—'}</div>
          <div className="text-xs text-slate-400 mt-1">BRL/USD · PTAX BCB</div>
        </div>
        {/* Bitcoin */}
        <div className="bg-card border border-slate-700/50 rounded-lg p-3 text-center">
          <div className="text-xs uppercase font-semibold text-slate-400 mb-1 tracking-widest">Bitcoin</div>
        ...
        {/* IPCA+ 2040 */}
        <div className="bg-card border border-slate-700/50 rounded-lg p-3 text-center">
          <div className="text-xs uppercase font-semibold text-slate-400 mb-1 tracking-widest flex items-center justify-center gap-1">
            IPCA+ 2040 — Taxa
            <span
          ...
          <div className="text-xl font-black text-white">{ipcaTaxa ? `${ipcaTaxa.toFixed(2)}%` : '—'}</div>
        ...
        <div className="bg-card border border-slate-700/50 rounded-lg p-3 text-center">
          ...
          <div className="text-xl font-black text-white">{rendaTaxa ? `${rendaTaxa.toFixed(2)}%` : '—'}</div>
          <div className="text-xs text-slate-400 mt-1">
      ...
      {data?.fire_matrix?.by_profile && Array.isArray(data.fire_matrix.by_profile) && (
        <div className="grid grid-cols-3 gap-2 mb-3">
          {data.fire_matrix.by_profile.map((profile: any, i: number) => {
            ...
            return (
              <div key={i} className="bg-slate-700/30 border-t-2 border-blue-500/40 rounded-lg p-2.5 text-center">
                <div className="text-xs uppercase font-semibold text-slate-400 mb-1 tracking-widest">
                  {labels[i]}
                </div>
                <div className="text-sm font-bold text-blue-400">FIRE 50</div>
                <div className="text-sm font-bold text-green-400 mt-0.5">
                  P = {pfire50 != null ? `${pfire50.toFixed(1)}%` : '—'}
                </div>
                <div className="text-xs text-slate-400 mt-1">{year50}</div>
              </div>
            );
          })}
        </div>
        ...
        <section className="bg-card border border-border/50 rounded-lg p-4 mb-3.5">
          <div className="flex gap-5 items-start">
            ...
            <div className="min-w-32 text-center">
              <div className="text-xs uppercase font-semibold text-slate-400 mb-1.5 tracking-widest">
                Financial Wellness Score <span className="italic text-xs">(indicador secundário)</span>
              </div>
              <div className="text-5xl font-black text-green-400 leading-none">
                {Math.round(derived.wellnessScore * 100)}
              </div>
              <div className="text-xs text-slate-400 mt-1">/100 · Progressivo</div>
            </div>
            {/* Barras de métricas */}
            <div className="flex-1">
              {derived.wellnessMetrics.slice(0, 8).map((m: any, i: number) => (
                <div key={i} className="mb-1.5 flex items-center gap-2">
                  <div className="text-xs text-slate-400 w-40 flex-shrink-0">{m.label}</div>
      ...
      <CollapsibleSection id="section-macro" title="Contexto Macro & DCA Status" defaultOpen={true} icon="📊">
        <div className="px-4 pb-4">
          ...
          <div className="mb-3.5">
            <div className="text-xs uppercase font-semibold text-slate-400 mb-2 tracking-widest">
              Exposição Brasil
            </div>
            <div className="bg-slate-700/40 rounded-lg p-3 mb-2">
              <div className="flex justify-between mb-2">
                <div>
                  <div className="text-xs text-slate-400">Total Brasil</div>
            ...
            <div className="mb-3.5">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                {data.dca_status.ipca_longo && (
                  <div className="bg-slate-700/40 rounded-lg p-2.5 border-l-3 border-blue-500/40">
                    <div className="text-xs text-slate-400 mb-1">IPCA+ 2040</div>
                ...
                {data.dca_status.ipca_medio && (
                  <div className="bg-slate-700/40 rounded-lg p-2.5 border-l-3 border-blue-500/40">
                    <div className="text-xs text-slate-400 mb-1">IPCA+ 2050</div>
                ...
                {data.rf?.renda2065?.distancia_gatilho && (
                  <div className="bg-slate-700/40 rounded-lg p-2.5 border-l-3 border-blue-500/40">
                    <div className="text-xs text-slate-400 mb-1">Renda+ 2065</div>
          ...
          <div className="mb-3.5">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-slate-700/40 rounded-lg p-2.5 text-center">
                <div className="text-lg font-bold text-white">{data?.premissas?.taxa_selic ? `${data.premissas.taxa_selic.toFixed(1)}%` : '—'}</div>
                <div className="text-xs text-slate-400 mt-1">Selic</div>
              </div>
              <div className="bg-slate-700/40 rounded-lg p-2.5 text-center">
                <div className="text-lg font-bold text-white">{data?.premissas?.ipca_corrente ? `${data.premissas.ipca_corrente.toFixed(1)}%` : '—'}</div>
                <div className="text-xs text-slate-400 mt-1">IPCA YTD</div>
              </div>
              <div className="bg-slate-700/40 rounded-lg p-2.5 text-center">
                <div className="text-lg font-bold text-white">{derived.CAMBIO ? `R$ ${derived.CAMBIO.toFixed(2)}` : '—'}</div>
                <div className="text-xs text-slate-400 mt-1">USD/BRL</div>
              </div>
            </div>
          </div>
