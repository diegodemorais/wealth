...
function CascadeSection() {
  ...
  // data.cascade.ipca_gap and data.cascade.renda_gap are not yet generated by scripts.
  // When absent, fall back to showing all allocation going to Equity with an explanatory label.
  const ipcaGap: number | null = data?.cascade?.ipca_gap ?? null;
  const rendaGap: number | null = data?.cascade?.renda_gap ?? null;
  const cascadeGapsAvailable = ipcaGap !== null || rendaGap !== null;
  ...
  return (
    <div className="section" style={{ marginTop: '16px' }}>
      ...
      {/* Cascade result — horizontal grid */}
      {!cascadeGapsAvailable ? (
        /* Graceful fallback: gaps not yet calculated by scripts */
        <div style={{ background: 'var(--card2)', borderRadius: '8px', padding: '14px', border: '1px solid var(--border)', borderTop: '3px solid var(--orange)', marginBottom: '10px' }}>
          <div style={{ fontSize: '.6rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: '6px' }}>
            Equity (aporte integral)
          </div>
          <div style={{ fontSize: '1.3rem', fontWeight: 700 }} className="pv">
            {fmtBRL(equityAlloc)}
          </div>
          <div style={{ fontSize: '.65rem', color: 'var(--muted)', marginTop: '6px' }}>
            Gaps IPCA+ e Renda+ serão calculados quando DCA estiver ativo (cascade.ipca_gap e cascade.renda_gap ausentes no data.json).
          </div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '10px', marginBottom: '10px' }}>
          <div style={{ background: 'var(--card2)', borderRadius: '8px', padding: '12px', border: '1px solid var(--border)', borderTop: '3px solid var(--green)' }}>
            <div style={{ fontSize: '.6rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: '6px' }}>
              IPCA+ 2040 (gap)
            </div>
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--green)' }} className="pv">
              {ipcaAlloc !== null ? fmtBRL(ipcaAlloc) : '—'}
            </div>
          </div>
          <div style={{ background: 'var(--card2)', borderRadius: '8px', padding: '12px', border: '1px solid var(--border)', borderTop: '3px solid var(--accent)' }}>
            <div style={{ fontSize: '.6rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: '6px' }}>
              Renda+ (gap)
            </div>
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--accent)' }} className="pv">
              {rendaAlloc !== null ? fmtBRL(rendaAlloc) : '—'}
            </div>
          </div>
